...
class SquareMap(wx.Panel):
    ...
    def OnMouse(self, event):
        ...
        node = HotMapNavigator.findNodeAtPosition(self.hot_map,
                                                  event.GetPosition())
...
#        self.DrawIconAndLabel(dc, node, x, y, w, h, depth)
        ...
        if w > self.padding * 2 and h > self.padding * 2:
            ...
            if children:
                ...
                self.LayoutChildren(dc, children, node, x, y, w, h,
                                    children_hot_map, depth + 1)
            else:
    ...
    def DrawIconAndLabel(self, dc, node, x, y, w, h, depth):
        ...
        if w - 2 < self._em_size_ // 2 or h - 2 < self._em_size_ // 2:
            return
        dc.SetClippingRegion(x + 1, y + 1, w - 2,
                             h - 2)  # Don't draw outside the box
        try:
        ...
        if total:
            if self.square_style and len(nodes) > 5:
                ...
                if head and tail:
                    ...
                    head_coord, tail_coord = split_box(head_sum / float(total),
                                                       x, y, w, h)
                    if head_coord:
                        self.LayoutChildren(
                            dc,
            ...
            head_coord, tail_coord = split_box(firstSize / float(total), x, y,
                                               w, h)
            if head_coord:
                self.DrawBox(dc, firstNode, head_coord[0], head_coord[1],
                             head_coord[2], head_coord[3], hot_map, depth)
            else:
...
def coord_bigger_than_padding(tail_coord, padding):
    return (tail_coord and tail_coord[2] > padding * 2 and
...
class DefaultAdapter(object):
    ...
    def empty(self, node):
        ...
        if overall:
            return (overall - self.children_sum(self.children(node), node)
                    ) / float(overall)
        return 0
...
class Node(object):
    ...
    def __repr__(self):
        return '%s( %r, %r, %r )' % (self.__class__.__name__, self.path,
